...
function getQuakeml($line) {
  ...
  $year = trim($line['year']);
  $month = trim($line['month']);
  $day = trim($line['day']);
  $hour = trim($line['hour']);
  $minute = trim($line['minute']);
  $second = trim($line['second']);
  ...
  $eventTime = sprintf('%\'04d-%\'02d-%\'02dT%\'02d:%\'02d:%\'06.3fZ',
      ...
      intval($minute),
      floatval($second));
  $latitude = trim($line['latitude']);
  $longitude = trim($line['longitude']);
  $depth = trim($line['depth']);
  $constraint = trim($line['epicenter constraint']);
  $feregion = trim($line['FE  region']);
  $precision = trim($line['precision']);
  $mmi = trim($line['MMI']);
  $felt = trim($line['felt']);
  ...
  if (trim($line['mag_1']) !== '') {
    $magnitudes[] = array(
      'magnitude' => trim($line['mag_1']),
      'source' => trim($line['mag_source_1']),
      'type' => trim($line['mag_type_1']),
      'id' => $magnitudePublicIDBase . '/mag_1');
  }
  if (trim($line['mag_2']) !== '') {
    $magnitudes[] = array(
      'magnitude' => trim($line['mag_2']),
      'source' => trim($line['mag_source_2']),
      'type' => trim($line['mag_type_2']),
      'id' => $magnitudePublicIDBase . '/mag_2');
  }
  if (trim($line['mb']) !== '') {
    $magnitudes[] = array(
      'magnitude' => trim($line['mb']),
      'source' => 'us',
      'type' => 'mb',
...
<q:quakeml xmlns="http://quakeml.org/xmlns/bed/1.2"' .
  ...
  foreach ($magnitudes as $mag) {
    ...
    $q .= '
      ...
      <creationInfo>
        <agencyID>sra</agencyID>
        <author>C.W. Stover, B.G. Reagor, and S.T. Algermissen</author>
      </creationInfo>
    </event>
    <creationInfo>
